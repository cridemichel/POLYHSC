...
\st{The preparation of  nanomaterials with defined shape and functionalisation, requires  control on the synthetic process.
...
\textcolor{red}{qui serve una frase di giuntura}
...
Effects of polydispersity have been already investigated both experimentally, theoretically 
and numerically in some cases; monodisperse hard spherocylinders of length $L$ and diameter $D$ have been studied by
means of computer simulations by Bolhuis and Frenkel~\cite{Bolhuis1997} and by McGrother~\cite{McGrother96}.
In these studies it has been shown that if elongation $A=L/D$ is greater than $\approx 3.7$ 
on increasing the concentration the following four stable phases can be found: isotropic (I), nematic (N), 
smectic (Sm), and uniaxial columnar crystal (K). Differently, if elongation is smaller then $3.7$ nematic and smectic phases 
are missing. 
...
% \begin{comment}
%         is the mutual excluded volume between two spherocyl\inders with respective orientations $\Omega_1$ and $\Omega_2$, $f(\Omega)$ is the orientational distribution function, and $\overline{v_0}$ is the average volume of the particles in the system.
	
%         The total free energy per particle is thus:
%         \begin{equation}
%                 \frac{\beta F}{N} = \ln(\Lambda^3 \rho) - 1 + \int f(\Omega) \ln(4 \pi f(\Omega)) d \Omega + \frac{\phi \, \eta(\phi)}{2 \, \overline{v_{0}}} \iint v_\mathrm{excl} (\Omega_1, \Omega_2) f(\Omega_1)f(\Omega_2) d\Omega_1 d\Omega_2
%         \end{equation}
	
%         where $\Lambda$ is the de Broglie thermal wavelength and $\rho = N/V$ is the number density. The integral over the last term represents the average excluded volume $\overline{v_\mathrm{excl}}$ between the particles in the system, and depends on the size distribution of the particles in the system and on their mutual orientation.
% \	
%         The reduced pressure $P^*$ of the system can be obtained from the free energy as:
	
%         \begin{equation}
%                 P^* =  P^*_\mathrm{id} - \beta \, \overline{v_0} \frac{\partial F_\mathrm{excl}}{\partial V} \rvert _{N, T}  = \phi - N \beta \, \overline{v_0} \frac{\overline{v_\mathrm{excl}}}{8 \, \overline{v_0}} \frac{\partial}{\partial V} \left( \phi \frac{4 - 3 \phi}{(1 - \phi)^2} \right) \nonumber = \phi + \frac{\overline{v_\mathrm{excl}}}{4 \, \overline{v_0}} \phi^2 \frac{2 - \phi}{(1 - \phi)^3} \label{ParsonLee}
%         \end{equation}
	
%         where $\tilde{v} = \overline{v_\mathrm{excl}} / \overline{v_0}$ represents the deviation from the ideal gas behaviour.
% \end{comment}
